...
fn test_unreadable_file_prints_error_and_continues() {
    ...
    assert!(stdout.contains("ERROR"), "Expected ERROR in stdout for unreadable file");
    assert!(stdout.contains("bad.rs"), "Expected filename in error message");

    // Verify process continued processing and indexed good.rs
    assert!(stdout.contains("MODIFY"), "Expected MODIFY event for good.rs");
    ...
    assert_eq!(symbols.len(), 2, "Should have indexed 2 symbols from good.rs despite bad.rs error");
}
