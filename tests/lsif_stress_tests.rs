...
fn test_lsif_import_missing_package() {
    ...
    let mut file = File::create(&lsif_path).unwrap();
    writeln!(file, r#"{{"type":"document","id":"d1","label":"document","uri":"test.rs"}}"#).unwrap();
...
fn create_test_lsif(path: &std::path::Path, symbol_count: usize) {
    ...
    for i in 0..symbol_count {
        ...
        // Symbol vertex
        writeln!(file, r#"{{"type":"symbol","id":"{}","label":"symbol","kind":"function"}}"#, symbol_id).unwrap();
        
        // Range vertex
        writeln!(file, r#"{{"type":"range","id":"{}","label":"range","range":[{},{},{},{}]}}"#, 
            range_id, line, character, line, character + 10).unwrap();
