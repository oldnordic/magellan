...
fn make_test_span(
    ...
    let (start_line, start_col) = byte_offset_to_line_col(source, byte_start)
        .expect("Invalid start offset");
    let (end_line, end_col) = byte_offset_to_line_col(source, byte_end)
...
fn test_half_open_span_extraction() {
    ...
    assert_eq!(extracted, Some("main"), "Half-open extraction should get 'main'");
}
...
fn test_span_length_equals_byte_end_minus_start() {
    ...
    assert_eq!(span.byte_end - span.byte_start, 4, "Length should be end - start");
    assert_eq!(span.byte_end - span.byte_start, "main".len(), "Length should match content");
}
...
fn test_adjacent_spans_no_overlap() {
    ...
    assert_eq!(span1.byte_end, span2.byte_start, "Adjacent spans should meet exactly");
    ...
    assert_eq!(combined, "fn main() ", "Adjacent spans should concatenate without gap");
}
...
fn test_empty_span_valid() {
    ...
    assert_eq!(span.byte_start, span.byte_end, "Empty span has start == end");
    assert_eq!(span.byte_end - span.byte_start, 0, "Empty span length is 0");

    // Empty slice is valid
    let extracted = source.get(span.byte_start..span.byte_end);
    assert_eq!(extracted, Some(""), "Empty span should extract empty string");
}
...
fn test_end_position_exclusive() {
    ...
    assert_eq!(span.end_line, 1, "End line should be same as start for single-line span");
    assert_eq!(span.end_col, 7, "End column should point to '(' (after 'main')");

    // Verify: the character at end_col is NOT included
    let char_after = source.chars().nth(span.byte_end);
    assert_eq!(char_after, Some('('), "Character at byte_end is NOT included in span");
}
...
fn test_span_extract_with_newlines() {
    ...
    let extracted = source.get(span.byte_start..span.byte_end).unwrap();
    assert_eq!(extracted, "e1\nline2\nli", "Should include newlines correctly");
}
...
fn test_span_bytes_vs_characters() {
    ...
    assert_eq!(span_ascii.byte_end - span_ascii.byte_start, 4, "ASCII length = 4 bytes");

    let extracted_ascii = source.get(span_ascii.byte_start..span_ascii.byte_end).unwrap();
    ...
    assert_eq!(span_cjk.byte_end - span_cjk.byte_start, 3, "CJK char = 3 bytes");
...
fn test_span_extraction_with_tabs() {
    ...
    assert_eq!(extracted.as_bytes().get(0), Some(&b'\t'), "First char is tab (byte 0x09)");
}
...
fn test_span_overlapping_validation() {
    ...
    assert_eq!(span1.byte_end, span2.byte_start, "Adjacent spans don't overlap");

    // Non-overlapping spans: neither contains the other's start
    assert!(span1.byte_end <= span2.byte_start, "span1 ends before or at span2 start");
}
...
fn test_span_roundtrip_conversion() {
    ...
    for (start, end) in test_offsets {
        ...
        let recovered_end = line_col_to_byte_offset(source, end_line, end_col)
            .expect("Failed to recover end");

        assert_eq!(recovered_start, start,
            "Roundtrip failed: start {} became {}", start, recovered_start);
        assert_eq!(recovered_end, end,
            "Roundtrip failed: end {} became {}", end, recovered_end);
    }
}
...
fn test_multibyte_column_is_byte_based() {
    ...
    assert!(result.is_some(), "Should return result for byte offset 6 (after CJK char)");

    let (line, col) = result.unwrap();
    assert_eq!(line, 0, "Still on line 0");
    assert_eq!(col, 6, "Column is byte offset (6), not character offset (4)");
    ...
    assert_eq!(byte.unwrap(), &0xe4, "Byte 3 is start of multi-byte CJK char (0xe4)");
}
...
fn test_byte_offset_beyond_source_returns_none() {
    ...
    assert!(byte_offset_to_line_col(source, 100).is_none(),
        "Offset beyond source should return None");
    assert!(byte_offset_to_line_col(source, source.len() + 1).is_none(),
...
fn test_line_col_beyond_source_returns_none() {
    ...
    assert!(line_col_to_byte_offset(source, 10, 0).is_none(),
...
fn test_span_id_integration() {
    ...
    assert_eq!(span.span_id.len(), 16, "Span ID should be 16 hex characters");
    ...
    assert_eq!(span.span_id, span2.span_id, "Same span data produces same ID");
}
