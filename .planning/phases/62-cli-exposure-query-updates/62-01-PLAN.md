...
wave: 1
depends_on: []
files_modified: [src/refs_cmd.rs, src/find_cmd.rs, src/query_cmd.rs, tests/backend_migration_tests.rs]
autonomous: true
...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/query_cmd.rs"
      provides: "CLI query command with cross-file caller/callee relationships"
      exports: ["run_query", "output_json_mode"]
      covered_by: "Task 4"
  ...
  key_links:
    ...
    - from: "src/query_cmd.rs"
      to: "cross-file caller/callee queries"
      via: "callers_of_symbol/calls_from_symbol graph methods"
      pattern: "callers_of_symbol|calls_from_symbol"
---
...
<task type="auto">
  <name>Task 4: Implement caller/callee references in query command</name>
  <files>src/query_cmd.rs</files>
  <action>
  Implement the TODO at src/query_cmd.rs:45-46 for caller/callee references:

  1. Remove the unused parameters _with_callers and _with_callees (lines 45-46):
     - Currently marked as TODO with comment "Implement caller/callee references"
     - Change _with_callers to with_callers (remove underscore)
     - Change _with_callees to with_callees (remove underscore)

  2. Add caller/callee queries to output_json_mode function:
     - When with_callers is true, query callers_of_symbol for each symbol
     - When with_callees is true, query calls_from_symbol for each symbol
     - Include results in SymbolMatch output (extend structure to include callers/callees)
     - Follow the pattern used in refs_cmd.rs for cross-file reference display

  3. For JSON/human output modes:
     - Callers: Show "Called from: function_name at path/to/file.rs:line"
     - Callees: Show "Calls: function_name at path/to/file.rs:line"
     - Each entry should include file_path to show cross-file relationships

  4. Update SymbolMatch or QueryResponse to include caller/callee data:
     - Add optional callers: Vec<Reference> field
     - Add optional callees: Vec<Reference> field
     - Each Reference should have: name, file_path, span information

  5. Remove TODO comments at lines 45-46 once implemented

  This enables query command to show cross-file call relationships,
  complementing the refs command which is reference-focused rather than symbol-focused.
  </action>
  <verify>cargo build --bin magellan</verify>
  <verify>magellan query --db test.db --file src/main.rs --with-callers --with-callees</verify>
  <done>query command includes cross-file caller/callee relationships when requested</done>
</task>
...
- refs command shows references from all files in codebase
- find command returns multi-file results with correct locations
- query command includes cross-file caller/callee relationships (via --with-callers/--with-callees flags)
- Cross-file reference tests pass (tests/backend_migration_tests.rs:648-987)
</verification>
...
6. Manual verification: query --with-callers shows callers from multiple files
7. Manual verification: query --with-callees shows callees in multiple files
</success_criteria>
