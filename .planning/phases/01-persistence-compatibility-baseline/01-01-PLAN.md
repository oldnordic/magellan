...
Purpose: Pin Magellan’s persistence layer to the latest published sqlitegraph so compatibility rules are enforced against a known, reproducible schema version.
...
<task type="auto">
  ...
  <action>
- Regenerate/refresh `Cargo.lock` so it resolves sqlitegraph v1.0.0 from crates.io (deterministically).
...
<verification>
- Confirm `cargo tree -i sqlitegraph` shows sqlitegraph v1.0.0 from the crates.io registry (no path override).
- Confirm build and tests pass under default features and `native-v2` feature.
</verification>

<success_criteria>
- Magellan can be built and tested while using sqlitegraph v1.0.0 pinned from crates.io (DB-01 satisfied).
</success_criteria>
