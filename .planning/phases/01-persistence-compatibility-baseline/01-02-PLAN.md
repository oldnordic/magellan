...
- Contingency branch (ONLY if `schema::SCHEMA_VERSION` is not usable from Magellan due to visibility/re-export/version drift):
  - Open the DB in READ_ONLY and read *the expected* schema version deterministically via raw SQL against a sqlitegraph-created DB shape.
  - Implementation constraint: this MUST still be deterministic and MUST NOT depend on unstable error strings.
