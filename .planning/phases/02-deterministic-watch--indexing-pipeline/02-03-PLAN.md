...
<task type="auto">
  ...
  <action>
    - Add WATCH-03 filtering support: **gitignore-style ignore rules + CLI include/exclude globs**.
      - Dependencies:
        - Add the `ignore` crate (gitignore-style matching) in Cargo.toml.
      ...
      - Sources of ignore rules (deterministic order):
        ...
        3) **Do not** read global gitignore by default (to keep watch results reproducible across machines). If you decide to support it, it MUST be behind an explicit CLI flag (e.g., `--use-global-gitignore`) and covered by tests.
    ...
    - Add tests/ignore_rules_tests.rs with at least:
      1) `.gitignore` and `.ignore` are honored (e.g., ignore `ignored_dir/**`) and record `Skipped` diagnostics with a reason like `IgnoredByGitignore` (add to SkipReason).
