...
must_haves:
  ...
  artifacts:
    ...
    - path: tests/indexer_determinism_tests.rs
      provides: "Regression tests proving no ghost refs/calls across reindex"
  key_links:
    - from: src/indexer.rs
      to: src/graph/ops.rs
...
<task type="auto">
  ...
  <action>
    ...
    - Do NOT rely on “edges cascade delete” for correctness here: the goal is to remove orphan nodes as well.
...
<task type="auto">
  ...
  <action>
    ...
    - Add `tests/indexer_determinism_tests.rs` with at least:
  ...
  <verify>
    - cargo test -q tests::indexer_determinism_tests -- --nocapture
    - cargo test -q
  </verify>
  <done>
