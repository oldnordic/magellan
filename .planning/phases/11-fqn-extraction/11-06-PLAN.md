...
must_haves:
  truths:
    ...
    - "Old databases are rejected with migration message"
  artifacts:
...
<objective>
Complete the FQN implementation by ensuring symbol_id generation uses the proper FQN (not simple name), SymbolFact stores FQN, and integration tests verify end-to-end correctness. Handle the breaking change by rejecting old databases.

Purpose: The symbol_id is the stable identifier used throughout the graph. It must be derived from the FQN to prevent collisions. This plan also adds integration tests and handles the database migration (forced re-index).
...
- Database version bump for forced re-index
</objective>
