...
impl ReferenceOps {
    ...
    pub fn index_references(
        ...
        // Extract references
        let references = parser.extract_references(PathBuf::from(path), source, &symbol_facts);
        ...
        for reference in &references {
            if let Some(&target_symbol_id) = symbol_ids.get(&reference.referenced_symbol) {
                ...
                self.insert_references_edge(reference_id, NodeId::from(target_symbol_id))?;
            }
        }
    ...
    fn insert_references_edge(&self, reference_id: NodeId, symbol_id: NodeId) -> Result<()> {
