...
pub fn parse_find_args(args: &[String]) -> Result<Command> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--glob" | "--list-glob" => {
                let value = parse_required_arg(args, &mut i, "--glob")?;
                glob_pattern = Some(value);
            }
            "--symbol-id" => {
                let value = parse_required_arg(args, &mut i, "--symbol-id")?;
                symbol_id = Some(value);
            }
            "--ambiguous" => {
                let value = parse_required_arg(args, &mut i, "--ambiguous")?;
                ambiguous_name = Some(value);
            }
            "--first" => {
                first = true;
                i += 1;
...
pub fn parse_refs_args(args: &[String]) -> Result<Command> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--direction" => {
                let value = parse_required_arg(args, &mut i, "--direction")?;
                direction = value;
...
pub fn parse_label_args(args: &[String]) -> Result<Command> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--label" => {
                let value = parse_required_arg(args, &mut i, "--label")?;
                label.push(value);
            }
            "--list" => {
                list = true;
...
pub fn parse_collisions_args(args: &[String]) -> Result<Command> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--field" => {
                let value = parse_required_arg(args, &mut i, "--field")?;
...
pub fn parse_migrate_backend_args(args: &[String]) -> Result<Command> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--input" => {
                let value = parse_required_arg(args, &mut i, "--input")?;
                input_db = Some(PathBuf::from(value));
            }
            "--output" => {
                let value = parse_required_arg(args, &mut i, "--output")?;
                output_db = Some(PathBuf::from(value));
            }
            "--export-dir" => {
                let value = parse_required_arg(args, &mut i, "--export-dir")?;
